A-Frame multi-user Multisynq component
====================================

The component allows easily, while staying in [A-Frame](https://aframe.io) scene definition (plain HTML file), to add a multi-user features onto A-Frame entities in scene. Create [Multisynq](https://multisynq.studio) (aka Multisynq V) simulations on A-Frame entities.  
A-Frame multi-user component works by synchronizing / replicating entities and their components to connected peers using Multisynq application architecture. It relies on public Multisynq reflectors, which are available online on the Internet.  
...
Add links to this component and multisynq lib in the html header

```html
<script src="https://unpkg.com/@multisynq/client"></script>
...
Since the demise of the Multisynq network and until and unless the Forerunner network is ready, **you'll need to run your own [open-source Croquet reflector](https://github.com/croquet/croquet/tree/files)**.
The `files` branch allows saving snapshots on the server, so you'll want to use that.

When using your own reflector, you don't use an `apiKey`; you set the `reflector` and `files` servers directly:
```html
<a-scene croquet reflector="wss://reflector.example.com" files="https://reflector.example.com/files">
```


By default, the session name and password will be generated by Multisynq app. Random key will be added in the url automatically, like:  
...
Multisynq free API key could be get from [https://multisynq.io/keys](https://multisynq.io/keys)
...
```  
Start a Multisynq application attached to an entity, by replicating a computation (animation example)
```js
...
During Multisynq initialization, the `data-seeds` attribute of the scene element will be set to a list of 25 random numbers - the same list on every client.
These numbers can be used for procedural generation of the world.

Multisynq Component Attributes
----------------------------
...
Whether the element will be animated by its Multisynq Model.
